...
///
/// There are some convenience classes to hold atomic types as values
/// HashString,HashDouble,HashInt
///
/// EXAMPLE:
///
/// <pre>
...
///      // 'true' means when the hash will adopt the values and delete them
///      when they are removed from the hash or when the hash is deleted.
///
///      h.hPut("a",a);
///      h.hPut("b",b);
///      h.hPut("c",c);
...
///    delete l;
///
///    // Delete from hash - This will also delete 'a' because we set the
///    'adopt' flag on the hash. h.hDel("a");
///
///    //Delete the hash - this will also delete 'b' and 'c' because of the
///    adopt flag. delete h;
/// </pre>
...
private:
  ...
  /// this should be approx number of elements to store. Depending on hash
  /// algorithm it may optimally be a prime or a power of two etc. but probably
  /// not important for MQL4 performance. A future optimisation might be to move
  /// the hashcode function to a DLL??
  uint _hashSlots;
  ...
  /// To store an object against a string(key) - get the string hashcode, then
  /// insert pair (key,val) into the linked list for that hashcode. To fetch an
  /// object against a string(key) - get the string hashcode, get linked-list at
  /// that hashcode index, then search for the key and return the val.
  HashEntry *_buckets[];

  /// If true the hash will free(delete) values as they are removed, or at
  /// cleanup.
  bool _adoptValues;
  ...
  // Hash table distribution is better when size is prime, eg if hash function
  // procduces numbers that are multiples of x, then there may be grouping
  // occuring around gcd(x,slots) gcd(2x,slots) etc using a prime size helps
  ...
  /// Primes that approx double in size, used for hash table sizes to avoid gcd
  /// causing bunching
  static uint _primes[];

  /// After reviewing quite a few hash functions I settled on the one below.
  /// http://www.cse.yorku.ca/~oz/hash.html
  /// this is the bottleneck function. Shame mql hash no default hash method for
  /// objects.
  uint hash(string s) {
...
public:
  ...
  /// Constructor: Create a Hash Object
  /// @param adoptValues : If true the hash destructor will <b>delete</b> all
  /// dynamically allocated hash values.
  Hash(bool adoptValues) { init(17, adoptValues); }

  /// Constructor: Create a Hash Object
  /// @param size : Approximate size (actual size will be a larger prime number
  /// close to a power of 2)
  /// @param adoptValues : If true the hash destructor will <b>delete</b> all
  /// dynamically allocated hash values.
  Hash(int size, bool adoptValues) { init(size, adoptValues); }
  ...
  ~Hash() {
    ...
    for (uint i = 0; i < _hashSlots; i++) {
      ...
      for (HashEntry *entry = _buckets[i]; entry != NULL; entry = nextEntry) {
        ...
        if (_adoptValues && entry._val != NULL &&
            CheckPointer(entry._val) == POINTER_DYNAMIC) {
          delete entry._val;
        }
        delete entry;
      }
  ...
  string getErrText() { return _errText; }

  /// If true the hash destructor will <b>delete</b> all dynamically allocated
  /// hash values.
  void setAdoptValues(bool v) { _adoptValues = v; }

  /// True if the hash destructor will <b>delete</b> all dynamically allocated
  /// hash values.
  bool getAdoptValues() { return _adoptValues; }

private:
  uint _foundIndex;       // After find() is called is set to hashindex for name
                          // whether found or not.
  HashEntry *_foundEntry; // After find() is called  is set to the HashEntry
                          // that contains the key.
  HashEntry *_foundPrev;  // After find() is called  is set to the HashEntry
                         // before the entry (could use double linked list but
  ...
  bool find(string keyName) {
    ...
    if (_foundIndex > _hashSlots) {
      ...
      setError(1, "hGet: bad hashIndex=" + (string)_foundIndex + " size " +
                      (string)_hashSlots);
      ...
      for (HashEntry *e = _buckets[_foundIndex]; e != NULL; e = e._next) {
        ...
        // Track the item before the target item in case deleting from single
        // linked list.
        _foundPrev = e;
      }
    }
...
public:
  ...
  HashEntry *getEntry(int i) { return _buckets[i]; }

  /// Return the number of slots/buckets (not number of elements)
  uint getSlots() { return _hashSlots; }
  /// Return the number of elements in the Hash
  int getCount() { return _hashEntryCount; }
  ...
  bool rehash(uint newSize) {
    ...
    // info("rehashing from "+(string)_hashSlots+" to "+(string)newSize+"
    // "+(string)GetTickCount());
    ...
    if (newSize <= getSlots()) {
      ...
      _resizeThreshold =
          (int)_hashSlots / 4 *
          3; // Just use the default load factor value of Javas HashTable
  ...
  bool hContainsKey(string keyName) { return find(keyName); }

  /// Fetch a value using string key
  ///  @return :HashValue associated with the key (or NULL if none found)
  ///  If the Hashtable contains legitimate NULL values then also check
  ///  errCode() Examples:
  ...
  /// Convenience method for getting values from a HashString value (see
  /// hPutString())
  ...
  /// Convenience method for getting values from a HashDouble value (see
  /// hPutDouble())
  ...
  /// Convenience method for getting  values from a HashDatetime ( see
  /// hPutDatetime())
  ...
  /// Store a hash value against the <b>keyName</b> key. This will overwrite any
  /// existing value. It adoptValues is set, it will also free the value if
  /// applicable.
  ...
  HashValue *hPut(string keyName, HashValue *obj) {
    ...
    if (find(keyName)) {
      ...
      if (_adoptValues && _foundEntry._val != NULL &&
      CheckPointer(_foundEntry._val) == POINTER_DYNAMIC ) { delete
      _foundEntry._val;
      }
      */
  ...
  bool hDel(string keyName) {
    ...
    if (find(keyName)) {
      ...
      if (_adoptValues && _foundEntry._val != NULL &&
          CheckPointer(_foundEntry._val) == POINTER_DYNAMIC) {
