...
public:
  ...
  JSONString() { setType(JSON_STRING); }
...
public:
  JSONObject() { setType(JSON_OBJECT); }
  ~JSONObject() {
    if (_hash != NULL)
      delete _hash;
  }

  string getString(string key) { return getValue(key).getString(); }
  bool getBool(string key) { return getValue(key).getBool(); }
  double getDouble(string key) { return getValue(key).getDouble(); }
  long getLong(string key) { return getValue(key).getLong(); }
  int getInt(string key) { return getValue(key).getInt(); }

  string operator[](string s) { return getValue(s).getString(); }

  bool getString(string key, string &out) {
    return getString(getValue(key), out);
  }
  bool getBool(string key, bool &out) { return getBool(getValue(key), out); }
  bool getDouble(string key, double &out) {
    return getDouble(getValue(key), out);
  }
  bool getLong(string key, long &out) { return getLong(getValue(key), out); }
  bool getInt(string key, int &out) { return getInt(getValue(key), out); }

  JSONArray *getArray(string key) { return getValue(key); }
  JSONObject *getObject(string key) { return getValue(key); }
  ...
  string toString() {
    ...
    if (_hash != NULL) {
      ...
      for (l = new HashLoop(_hash); l.hasNext(); l.next()) {
        ...
        StringConcatenate(s, s, (++n == 1 ? "" : ","), "\"", l.key(),
                          "\" : ", v.toString());
      }
      delete l;
    }
...
public:
  ...
  string getString(int index) { return getValue(index).getString(); }
  bool getBool(int index) { return getValue(index).getBool(); }
  double getDouble(int index) { return getValue(index).getDouble(); }
  long getLong(int index) { return getValue(index).getLong(); }
  int getInt(int index) { return getValue(index).getInt(); }

  bool getString(int index, string &out) {
    return getString(getValue(index), out);
  }
  bool getBool(int index, bool &out) { return getBool(getValue(index), out); }
  bool getDouble(int index, double &out) {
    return getDouble(getValue(index), out);
  }
  bool getLong(int index, long &out) { return getLong(getValue(index), out); }
  bool getInt(int index, int &out) { return getInt(getValue(index), out); }

  JSONArray *getArray(int index) { return getValue(index); }
  JSONObject *getObject(int index) { return getValue(index); }

  JSONValue *getValue(int index) { return _array[index]; }
...
public:
  ...
  JSONValue *parse(string s) {
    ...
    if (_len != inLen + 1 /* nul */) {
      StringConcatenate(_errMsg, "unable to create array ", inLen, " got ",
                        _len);
      setError(1, _errMsg);
    } else {
      _len--;
      ret = parseValue();
      if (_errCode != 0) {
        StringConcatenate(_errMsg, _errMsg, " at ", _pos, " [",
                          StringSubstr(_instr, _pos, 10), "...]");
      }
    }
    return ret;
  }
  ...
  void skipSpace() {
    while (_in[_pos] == ' ' || _in[_pos] == '\t' || _in[_pos] == '\r' ||
           _in[_pos] == '\n') {
      if (_pos >= _len)
        break;
      _pos++;
  ...
  bool expect(ushort c) {
    ...
    if (c == _in[_pos]) {
      ...
      StringConcatenate(_errMsg, "expected ", ShortToString(c), "(", c, ")",
  ...
  string parseString() {
    ...
    if (expect('"')) {
      while (true) {
        ...
        if (end >= _len) {
          setError(2, "missing quote: end" + (string)end + ":len" +
                          (string)_len + ":" + ShortToString(_in[_pos]) + ":" +
                          StringSubstr(_instr, _pos, 10) + "...");
          break;
        }
...
public:
  JSONIterator(JSONObject *jo) { _l = new HashLoop(jo.getHash()); }
  ~JSONIterator() { delete _l; }

  bool hasNext() { return _l.hasNext(); }

  void next() { _l.next(); }

  JSONValue *val() { return (JSONValue *)(_l.val()); }

  string key() { return _l.key(); }
};

void json_demo() {
  string s = "{ \"firstName\": \"John\", \"lastName\": \"Smith\", \"age\": 25, "
             "\"address\": { \"streetAddress\": \"21 2nd Street\", \"city\": "
             "\"New York\", \"state\": \"NY\", \"postalCode\": \"10021\" }, "
