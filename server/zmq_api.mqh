...
class ZMQ_api {
 ...
 public:
  ...
  void setup_server(string zeromq_protocol, string hostname, int rep_port, int pub_port);
  void close_server(string zeromq_protocol, string hostname, int rep_port, int pub_port);
  int listen_to_requests(ZmqMsg &_msg_container);
  void reply_to_requests(string reply);
  void publish(string topic, string data);
};

void ZMQ_api::setup_pub_server(string zeromq_protocol, string hostname, int pub_port) {
  Print("Binding PUB Server:" + (string)(pub_port) + "..");
  pub_socket.bind(
      StringFormat("%s://%s:%d", zeromq_protocol, hostname, pub_port));
}

void ZMQ_api::setup_rep_server(string zeromq_protocol, string hostname, int rep_port) {
...
void ZMQ_api::setup_server(string zeromq_protocol, string hostname,
                           int rep_port, int pub_port) {
  setup_rep_server(zeromq_protocol, hostname, rep_port);
  setup_pub_server(zeromq_protocol, hostname, pub_port);
}
